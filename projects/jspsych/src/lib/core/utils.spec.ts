...
describe('Check Key', function () {
  it('accept character', function () {
    let arr = ["a", "A", "c", "z", "1", "2", "3"];
    for (let c of arr) {
      expect(utils.isValidKey(c)).toBe(true);
    }
  });
  it('accept nonprintable keys', function () {
    let arr = ["Alt", "ArrowLeft"];
    for (let c of arr) {
      expect(utils.isValidKey(c)).toBe(true);
    }
  });
  it('reject', function () {
    let arr = ["abc", "123"];
    for (let c of arr) {
      expect(utils.isValidKey(c)).toBe(false);
    }
  });
})
